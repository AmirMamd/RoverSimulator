...
def perception_step(Rover):
    # PerfoPerformrm perception steps to update Rover()
    # TODO:
    ...
    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 17] *= 0
    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 18] *= 0
    # i=50
    for i in range(15,160):
        threshed_navigable[:, (threshed_navigable.shape[1] // 2) + i] *= 0

    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 50] *= 0
    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 51] *= 0
    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 52] *= 0
    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 53] *= 0
    # threshed_navigable[:, (threshed_navigable.shape[1] // 2) + 54] *= 0
    ...
    if (np.float(np.abs(Rover.roll) % 360) <= 3) and (np.float(np.abs(Rover.pitch) % 360) <= 3):
        Rover.worldmap[ypix_world_obs, xpix_world_obs, 0] += 10
